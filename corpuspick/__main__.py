...
class App:
    def __init__(self, window):
        ...
        for column, label, fn in [(3, 'По похожести', self.sort_similar),
                                  (4, 'Убрать префикс', self.trim_names)]:
    ...
    def refreshed(self):
        ...
        self.status.set(f"Файлов: {len(docs)} · SHA-256: {sum(bool(d.get('hash')) for d in docs)}/{len(docs)} · "
                        f"Ошибок: {sum(bool(d.get('error') or d.get('stats', {}).get('failed') or d.get('content', {}).get('failed') or d.get('content', {}).get('text_failed') or d.get('content', {}).get('topic_failed') or d.get('content', {}).get('visual_failed')) for d in docs)}" +
    ...
    def render(self):
        ...
        for number, d in ordered:
            ...
            if self.search.get().casefold() not in (d['path'] + ' ' + ' '.join(origins)).casefold():
                continue
            f = self.filter.get()
            if f == 'Точные дубли' and not d.get('duplicate') or f == 'Есть ошибки' and not (d.get('error') or d.get('stats', {}).get('failed') or d.get('content', {}).get('failed') or d.get('content', {}).get('text_failed') or d.get('content', {}).get('topic_failed') or d.get('content', {}).get('visual_failed')):
                continue
            stats = d.get('stats', {})
            def metric(name):
            ...
            if self.similarity_groups and d['path'] in self.similarity_groups:
                ...
                row_tags = (tag,)
            elif d.get('error') or stats.get('failed'):
                row_tags = ('error',)
    ...
    def sort_similar(self):
        ...
        def done(ranks):
            if ranks is not None:
                ...
                self.render()
                unavailable = sum(bool(d.get('content', {}).get('failed') or d.get('content', {}).get('text_failed') or d.get('content', {}).get('topic_failed') or d.get('content', {}).get('visual_failed')) for d in self.view_docs)
                from .content_similarity import size_only
                large = sum(size_only(d) for d in self.view_docs)
                ml_paths = [path for path, note in self.similarity_notes.items() if 'Тематическая группа ML' in note]
                ml_text = (f"ML-тем: {len({self.similarity_groups[path] for path in ml_paths})} · В темах: {len(ml_paths)} · "
                           if ml_paths else '')
                self.status.set(f'{ml_text}Сначала группы по содержимому, затем резервные по названию, типу и размеру · '
